...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/tickets/ticketsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureTicketsCols';
...
const TableSampleTickets = ({
  ...
  useEffect(() => {
    if (ticketsNotify.showNotification) {
      notify(ticketsNotify.typeNotification, ticketsNotify.textNotification);
    }
  }, [ticketsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/tickets/tickets-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {tickets && Array.isArray(tickets) && !showGrid && (
        <ListTickets
          tickets={tickets}
          loading={loading}
