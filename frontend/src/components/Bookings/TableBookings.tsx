...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/bookings/bookingsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureBookingsCols';
...
const TableSampleBookings = ({
  ...
  useEffect(() => {
    if (bookingsNotify.showNotification) {
      notify(bookingsNotify.typeNotification, bookingsNotify.textNotification);
    }
  }, [bookingsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/bookings/bookings-view/?id=${id}`);
  };

  const handleCreateEventAction = ({ start, end }: SlotInfo) => {
    router.push(
      `/bookings/bookings-new?dateRangeStart=${start.toISOString()}&dateRangeEnd=${end.toISOString()}`,
    );
  };
    ...
    <>
      ...
      {!showGrid && (
        <BigCalendar
          events={bookings}
          showField={'check_in'}
          start-data-key={'check_in'}
          end-data-key={'check_out'}
          handleViewAction={handleViewAction}
          handleEditAction={handleEditAction}
          handleDeleteAction={handleDeleteModalAction}
          pathEdit={`/bookings/bookings-edit/?id=`}
          pathView={`/bookings/bookings-view/?id=`}
