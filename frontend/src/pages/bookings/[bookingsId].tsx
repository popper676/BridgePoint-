...
const EditBookings = () => {
  ...
  const { bookingsId } = router.query;

  useEffect(() => {
    dispatch(fetch({ id: bookingsId }));
  }, [bookingsId]);
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: bookingsId, data }));
...
EditBookings.getLayout = function getLayout(page: ReactElement) {
  return (
