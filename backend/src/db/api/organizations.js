...
module.exports = class OrganizationsDBApi {
  ...
  static async findBy(where, options) {
    ...
    output.bookings_organization = await organizations.getBookings_organization(
      {
        transaction,
      },
    );

    output.feedback_organization = await organizations.getFeedback_organization(
      {
        transaction,
      },
    );

    output.promotions_organization =
      await organizations.getPromotions_organization({
        transaction,
      });

    output.purchases_organization =
      await organizations.getPurchases_organization({
        transaction,
      });

    output.rooms_organization = await organizations.getRooms_organization({
      transaction,
    });

    output.tickets_organization = await organizations.getTickets_organization({
      transaction,
    });

    output.socialmediadata_organization =
      await organizations.getSocialmediadata_organization({
        transaction,
      });

    output.revenue_organization = await organizations.getRevenue_organization({
      transaction,
    });
