...
module.exports = class PromotionsDBApi {
  static async create(data, options) {
    ...
    await promotions.setOrganization(currentUser.organization.id || null, {
      transaction,
    });
  ...
  static async bulkImport(data, options) {
    ...
    // Bulk create items
    const promotions = await db.promotions.bulkCreate(promotionsData, {
      transaction,
    });
  ...
  static async update(id, data, options) {
    ...
    await promotions.setOrganization(
  ...
  static async findBy(where, options) {
    ...
    const promotions = await db.promotions.findOne({ where }, { transaction });
    ...
    output.organization = await promotions.getOrganization({
      transaction,
    });
  ...
  static async findAll(filter, globalAccess, options) {
    ...
    if (filter) {
      ...
      if (filter.title) {
        where = {
          ...where,
          [Op.and]: Utils.ilike('promotions', 'title', filter.title),
        };
      }
      ...
      if (filter.details) {
        where = {
          ...where,
          [Op.and]: Utils.ilike('promotions', 'details', filter.details),
        };
      }
    ...
    let { rows, count } = options?.countOnly
      ? {
          rows: [],
          count: await db.promotions.count({
      ...
      : await db.promotions.findAndCountAll({
          where: globalAccess ? {} : where,
          include,
          distinct: true,
          limit: limit ? Number(limit) : undefined,
  ...
  static async findAllAutocomplete(query, limit, globalAccess, organizationId) {
    ...
    if (query) {
      where = {
        [Op.or]: [
          { ['id']: Utils.uuid(query) },
          Utils.ilike('promotions', 'title', query),
        ],
      };
    }
